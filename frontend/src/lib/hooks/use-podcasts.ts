...
export function usePodcastEpisodes(options?: { autoRefresh?: boolean; pollInterval?: number }) {
  const { autoRefresh = true, pollInterval } = options ?? {}
  ...
  const query = useQuery({
    ...
    refetchInterval: (current) => {
      ...
      // could miss an episode created right after the last fetch).
      if (pollInterval) {
        return pollInterval
      }
