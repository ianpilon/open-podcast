...
export default function TransformationsPage() {
  ...
  // v2: hide default transformations we don't want in the stripped-down app.
  // Records stay in the database (v1 unaffected); this only filters the UI list.
  const HIDDEN_TRANSFORMATIONS = ['Analyze Paper', 'Dense Summary', 'Key Insights', 'Reflections']
  const visibleTransformations = transformations?.filter(
    (tr) => !HIDDEN_TRANSFORMATIONS.includes(tr.name)
  )
